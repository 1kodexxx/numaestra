...
function FootLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link
      to={to}
      style={{ display: 'block', fontSize: '13px', color: TEXT2, textDecoration: 'none', padding: '5px 0', transition: 'color 0.15s', width: 'fit-content' }}
...
function ColTitle({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: '11px', fontWeight: 700, color: TEXT3, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>{children}</div>
}
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '44px 24px 0' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '40px', justifyContent: 'space-between' }}>
          ...
          <div style={{ maxWidth: 320 }}>
            <div className="brand-link" style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              <BrandMark size={26} />
        ...
        <div style={{
          ...
          borderTop: `1px solid ${BORDER}`, marginTop: '36px', padding: '18px 0 24px',
        }}>
          <span style={{ fontSize: '12px', color: TEXT3 }}>© {year} Numaestra · Персональные песни на заказ</span>
          <span style={{ fontSize: '12px', color: TEXT3, display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
            Работает на <span style={{ color: ACCENT, fontWeight: 600 }}>Suno AI</span>
          </span>
        </div>
      </div>
    </footer>
