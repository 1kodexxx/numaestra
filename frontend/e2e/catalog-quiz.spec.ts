...
test.describe('Каталог → конструктор → контакты', () => {
  ...
  test('открывает конструктор и модалку оформления', async ({ page }) => {
    ...
    // Заголовок модалки зависит от цен из /public/config: «Заказать песню» в
    // воронке «оплата сразу», «Послушать демо за N ₽» при платном демо и
    // «Получить бесплатное демо» при нулевой цене демо. Цену в шаблон не
    // включаем — тест не должен падать от смены тарифа.
    const dialog = page.getByRole('dialog', {
      name: /Заказать песню|Создаём заказ|Послушать демо|Получить бесплатное демо|Готовим демо/i,
    })
