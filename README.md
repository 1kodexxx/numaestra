...
**Suno** генерирует текст и музыку (Inspiration Mode), **Robokassa** принимает оплату, а
...
| Построение промпта | шаблон категории + ответы квиза → encoded Suno prompt на сервере | свободный бриф → encoded Suno prompt на сервере |
| Роль LLM | не используется — Suno Inspiration Mode | не используется — Suno Inspiration Mode |
...
| **Интеграции** | Suno API (Sunor.cc), Robokassa, S3, SMTP |
...
`SUNO_API_KEY`, `REDIS_PASSWORD`, `SESSION_ENCRYPTION_KEY`, `S3_ACCESS_KEY`/`S3_SECRET_KEY`,
