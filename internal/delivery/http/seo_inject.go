...
var legalDocs = map[string]struct{ title, description string }{
	...
	"reviews":   {"Правила отзывов", "Правила публикации отзывов о сервисе Numaestra: модерация и ограничения."},
}
...
func (s *SEOInjector) faqItems() []qa {
	...
	return append(items,
		qa{"Сколько времени занимает создание песни?", "Около 10 минут после оплаты. Нейросеть создаёт сразу 4 версии — вы выбираете лучшую."},
		qa{"Нужна ли регистрация?", "Нет. Достаточно описать повод и оставить email — ссылка на готовый трек придёт на почту."},
		qa{"В каком виде я получу песню?", "4 готовые версии в полном качестве, без водяного знака. Слушайте онлайн, скачивайте и делитесь — ссылки остаются у вас навсегда."},
	)
}
